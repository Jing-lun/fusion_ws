...
void PangolinViewer::Run()
{
    ...
    while(1)
    {
        ...
        if(save)
        {
            ...
            mpSystem->SaveMap("/home/robooster/poine_ws/src/poine_orbslam/map/mapdata.bin");
        ...
        if(load)
        {
            ...
            string mapaddress = "/home/robooster/poine_ws/src/poine_orbslam/map/mapdata.bin";
            ORB_SLAM2::ORBVocabulary voc;
            const string voc_file = "/home/robooster/viorb_config/config/ORBvoc.bin";
            voc.loadFromBinaryFile(voc_file);
            mpSystem->LoadMap( mapaddress, voc );
            std::chrono::steady_clock::time_point t4=std::chrono::steady_clock::now();
            double  tload=std::chrono::duration_cast<std::chrono::duration<double> >(t4 - t3).count();
            load=false;
        }
#endif
        if(menuReset)
